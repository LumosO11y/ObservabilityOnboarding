...
1. **Action**: Create a ConfigMap with a `BACKGROUND_COLOR` key set to `blue`.

2. **The Test**: Update the Deployment YAML to inject it as an environment variable, then exec into one of the pods and print the variable.

3. **The Challenge**: Change the ConfigMap value from `blue` to `red`. Print the variable in the same running pod again before doing anything else. Then trigger a rollout and print it in one of the new pods.

4. **The Question**: Why didn't the running pod pick up the new value on its own? And why is this whole approach still better than hardcoding the color inside the Dockerfile?
